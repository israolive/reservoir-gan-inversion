...
class FaciesGAN:
    def __init__(self, device: torch.device, options: argparse.Namespace | SimpleNamespace,
        ...
        self.generator = Generator(
            ...
            self.kernel_size,
            self.padding_size,
            self.facie_num_channels * 2, # Input: Noise (C) + Mask (C) = 2C
            self.facie_num_channels      # Output: Image (C)
        ).to(self.device)
    ...
    def get_noise(self,
        ...
        def generate_noise(index: int) -> torch.Tensor:
            ...
            if self.masked_facies is not None:
                # C channels noise
                z = ops.generate_noise((self.facie_num_channels, *shape), device=self.device, num_samp=len(mask_indexes))
                z = torch.cat([z, self.masked_facies[index][mask_indexes].to(self.device)], dim=1)
            else:
                 # 2*C channels noise (to match expected input size)
                 z = ops.generate_noise((self.facie_num_channels * 2, *shape), device=self.device, num_samp=len(mask_indexes))
