...
class Trainer:
    ...
    def __initialize_noise(self,
        ...
        if scale == 0:
            z_rec = generate_noise((self.model.facie_num_channels, *self.scales_list[scale][2:]), device=self.device, num_samp=self.batch_size)
            ...
            z_rec = torch.zeros((self.batch_size, self.model.facie_num_channels, *self.scales_list[scale][2:]), device=self.device)
            z_rec = torch.cat([z_rec, masked_facie], dim=1)
            z_rec = F.pad(z_rec, [self.zero_padding] * 4, value=0)
            update_rec_noise(z_rec)
